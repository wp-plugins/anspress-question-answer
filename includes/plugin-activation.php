...
if ( ! class_exists( 'ap_TGM_Plugin_Activation' ) ) {
    /**
     ...
     * @author  Gary Jones <gamajo.com>
     */
    class ap_TGM_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'ap_tgmpa-install-plugins';
         ...
         * does the ap_tgmpa_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
         * @see ap_TGM_Plugin_Activation::init()
         */
        public function __construct() {
            ...
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'ap_tgmpa' ),
                'menu_title'                     => __( 'Install Plugins', 'ap_tgmpa' ),
                'installing'                     => __( 'Installing Plugin: %s', 'ap_tgmpa' ),
                'oops'                           => __( 'Something went wrong.', 'ap_tgmpa' ),
                ...
                'return'                         => __( 'Return to Required Plugins Installer', 'ap_tgmpa' ),
                'dashboard'                      => __( 'Return to the dashboard', 'ap_tgmpa' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'ap_tgmpa' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'ap_tgmpa' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'ap_tgmpa' ),
                'dismiss'                        => __( 'Dismiss this notice', 'ap_tgmpa' ),
            );
            ...
            do_action_ref_array( 'ap_tgmpa_init', array( $this ) );
         ...
         * @since 2.0.0
         *
         * @see ap_TGM_Plugin_Activation::admin_menu()
         * @see ap_TGM_Plugin_Activation::notices()
         * @see ap_TGM_Plugin_Activation::styles()
         */
        public function init() {

            do_action( 'ap_tgmpa_register' );
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'ap_tgmpa_dismissed_notice', true ) ) {
                add_thickbox();
            }
         ...
         *
         * @see ap_TGM_Plugin_Activation::init()
         * @see ap_TGM_Plugin_Activation::install_plugins_page()
         */
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new ap_TGMPA_List_Table;

            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'ap_tgmpa-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }

            ?>
            <div class="ap_tgmpa wrap">
                ...
                <form id="ap_tgmpa-plugins" action="" method="post">
                    <input type="hidden" name="ap_tgmpa-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['ap_tgmpa-install'] ) && 'install-plugin' == $_GET['ap_tgmpa-install'] ) ) {
                check_admin_referer( 'ap_tgmpa-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'ap_tgmpa-install' => 'install-plugin',
                        ),
                ...
                $fields = array( 'ap_tgmpa-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'ap_tgmpa' ) . '">' . __( 'Return to the Dashboard', 'ap_tgmpa' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['ap_tgmpa-activate'] ) && 'activate-plugin' == $_GET['ap_tgmpa-activate'] ) ) {
                check_admin_referer( 'ap_tgmpa-activate', 'ap_tgmpa-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'ap_tgmpa' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'ap_tgmpa' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'ap_tgmpa' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'ap_tgmpa_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'ap_tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'ap_tgmpa', 'ap_tgmpa', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'ap_tgmpa', 'ap_tgmpa', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'ap_tgmpa' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['ap_tgmpa-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'ap_tgmpa_dismissed_notice', 1 );
            }
         ...
         */
        protected function is_ap_tgmpa_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'ap_tgmpa_dismissed_notice' );
         ...
         * @since 2.4.0
         *
         * @return object The ap_TGM_Plugin_Activation object.
         */
        public static function get_instance() {

            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof ap_TGM_Plugin_Activation ) ) {
                self::$instance = new ap_TGM_Plugin_Activation();
            }
    ...
    $ap_tgmpa = ap_TGM_Plugin_Activation::get_instance();

}

if ( ! function_exists( 'ap_tgmpa' ) ) {
    /**
     ...
     */
    function ap_tgmpa( $plugins, $config = array() ) {

        foreach ( $plugins as $plugin ) {
            ap_TGM_Plugin_Activation::$instance->register( $plugin );
        }

        if ( $config ) {
            ap_TGM_Plugin_Activation::$instance->config( $config );
        }
...
if ( ! class_exists( 'ap_TGMPA_List_Table' ) ) {
    /**
     ...
     */
    class ap_TGMPA_List_Table extends WP_List_Table {

        /**
         * References parent constructor and sets defaults for class.
         *
         * The constructor also grabs a copy of $instance from the TGMPA class
         * and stores it in the global object ap_TGM_Plugin_Activation::$instance.
         *
         * @since 2.2.0
         *
        ...
        protected function _gather_plugin_data() {
            ...
            ap_TGM_Plugin_Activation::$instance->admin_init();
            ap_TGM_Plugin_Activation::$instance->thickbox();
            ...
            foreach ( ap_TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                if ( is_plugin_active( $plugin['file_path'] ) ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'ap_tgmpa' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'ap_tgmpa' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'ap_tgmpa' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'ap_tgmpa' ) : __( 'Recommended', 'ap_tgmpa' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'ap_tgmpa' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'ap_tgmpa' ) );
                }
         ...
         * ap_TGM_Plugin_Activation class.
         *
         * Loops through the registered plugins looking for $name. If it finds it,
         * it returns the $data from that plugin. Otherwise, returns false.
         *
        ...
        protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
            ...
            foreach ( ap_TGM_Plugin_Activation::$instance->plugins as $plugin => $values ) {
                if ( $name == $values['name'] && isset( $values[$data] ) ) {
                    return $values[$data];
                }
            }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'ap_tgmpa' ) . ' %2$s">' . __( 'Install', 'ap_tgmpa' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
                                    'page'          => ap_TGM_Plugin_Activation::$instance->menu,
                                    'plugin'        => $item['slug'],
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'ap_tgmpa' ) . ' %2$s">' . __( 'Activate', 'ap_tgmpa' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => ap_TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'ap_tgmpa-activate'       => 'activate-plugin',
                                'ap_tgmpa-activate-nonce' => wp_create_nonce( 'ap_tgmpa-activate' ),
                            ),
                            network_admin_url( 'themes.php' )
                        ),
                        $item['sanitized_plugin']
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'ap_tgmpa' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'ap_tgmpa' ),
                'source' => __( 'Source', 'ap_tgmpa' ),
                'type'   => __( 'Type', 'ap_tgmpa' ),
                'status' => __( 'Status', 'ap_tgmpa' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'ap_tgmpa-bulk-install'  => __( 'Install', 'ap_tgmpa' ),
                'ap_tgmpa-bulk-activate' => __( 'Activate', 'ap_tgmpa' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'ap_tgmpa-bulk-install' === $this->current_action() ) {
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => ap_TGM_Plugin_Activation::$instance->menu,
                            'ap_tgmpa-action'  => 'install-selected',
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'ap_tgmpa-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'ap_tgmpa-empty';
                    $i++;
                }

                if ( is_wp_error( $api ) ) {
                    wp_die( ap_TGM_Plugin_Activation::$instance->strings['oops'] );
                }
                ...
                $url   = add_query_arg( array( 'page' => ap_TGM_Plugin_Activation::$instance->menu ), network_admin_url( 'themes.php' ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;

                // Create a new instance of ap_TGM_Bulk_Installer.
                $installer = new ap_TGM_Bulk_Installer( $skin = new ap_TGM_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );

                // Wrap the install process with the appropriate HTML.
                echo '<div class="ap_tgmpa wrap">';
            ...
            if ( 'ap_tgmpa-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'ap_tgmpa' ), $imploded );
                }
 ...
 * @since 2.2.0
 */
if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && ap_TGM_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
    require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';

    if ( ! class_exists( 'ap_TGM_Bulk_Installer' ) ) {
        /**
         ...
         * @author  Gary Jones <gamajo.com>
         */
        class ap_TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function bulk_install( $packages ) {
                ...
                if ( ap_TGM_Plugin_Activation::$instance->is_automatic ) {
            ...
            public function run( $options ) {
                ...
                if ( ap_TGM_Plugin_Activation::$instance->is_automatic ) {
                    ...
                    ap_TGM_Plugin_Activation::$instance->populate_file_path();
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'ap_tgmpa' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'ap_tgmpa' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'ap_tgmpa' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'ap_tgmpa' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'ap_tgmpa' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'ap_tgmpa' );
            ...
            public function activate_strings() {
                ...
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'ap_tgmpa' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'ap_tgmpa' );
    ...
    if ( ! class_exists( 'ap_TGM_Bulk_Installer_Skin' ) ) {
        /**
         ...
         * @author  Gary Jones <gamajo.com>
         */
        class ap_TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( ap_TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'ap_tgmpa' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'ap_tgmpa' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'ap_tgmpa' ) . '</span><span class="hidden">' . __( 'Hide Details', 'ap_tgmpa' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'ap_tgmpa' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'ap_tgmpa' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'ap_tgmpa' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'ap_tgmpa' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'ap_tgmpa' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'ap_tgmpa' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'ap_tgmpa' ) . '</span><span class="hidden">' . __( 'Hide Details', 'ap_tgmpa' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'ap_tgmpa' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'ap_tgmpa' );
                }
            ...
            public function bulk_footer() {
                ...
                foreach ( ap_TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                    if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                        echo '<p><a href="' . add_query_arg( 'page', ap_TGM_Plugin_Activation::$instance->menu, network_admin_url( 'themes.php' ) ) . '" title="' . esc_attr( ap_TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . ap_TGM_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                        $complete[] = $plugin;
                        break;
                    }
                    // Nothing to store.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( ap_TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'ap_tgmpa' ) . '">' . __( 'Return to the Dashboard', 'ap_tgmpa' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
